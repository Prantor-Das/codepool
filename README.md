...
### Daytona setup values

`DAYTONA_COMPOSE_TEMPLATE=docker-compose.yml` points to the repository-root `docker-compose.yml`. It is used by Daytona sandboxes, not by the normal local Next.js development flow. The Compose file defines the three services required by the sandbox validator: `app`, sandbox-local `postgres:5432`, and sandbox-local `redis:6379`.

The project currently has only `main` and `origin/main`; `feature/my-change` is an example name, not an existing branch. Use real revisions:
...
# Fetch a GitHub pull-request head into a local branch (replace 123)
git fetch origin pull/123/head:pr-123
git rev-parse pr-123
```

Use full 40-character SHA values from these commands for `DAYTONA_VERIFY_BASE_SHA` and `DAYTONA_VERIFY_PR_SHA`. For a local smoke test, `HEAD~1` and `HEAD` are valid candidates:

```bash
git rev-parse HEAD~1   # base
git rev-parse HEAD     # PR candidate
```

`DAYTONA_REPO_ARCHIVES_JSON` is a short-lived manifest generated by the trusted control plane. It maps each exact SHA to an HTTPS archive URL, an expiration time within ten minutes, and the archive SHA-256 digest. The sandbox validates the digest before extraction; GitHub checkout credentials must never enter the sandbox.

Keep the sandbox database and network settings as follows:

```env
DAYTONA_DATABASE_URL=postgresql://postgres:postgres@postgres:5432/postgres
DAYTONA_NETWORK_BLOCK_ALL=true
```

The database URL points to the Compose `postgres` service and must not be replaced with CodePool's production `DATABASE_URL`. External calls must be represented by the offline replay cassette.
...
Each target repository can define its own sandbox contract in `.codepool/sandbox.json` at the PR base commit:

```json
{
  "healthEndpoint": "/api/health",
  "fixturePath": "test/fixtures/sandbox.sql",
  "fixtureVersion": "v1",
  "replayCassettePath": "test/fixtures/replay.json",
  "scenarios": [
    { "id": "health", "method": "GET", "path": "/api/health", "iterations": 3 },
    { "id": "transfer", "method": "POST", "path": "/transfer", "body": { "from": "alice", "to": "bob", "amount": 10 }, "iterations": 20 }
  ]
}
```

`healthEndpoint` documents the repository's health route; each scenario is an API route plus optional request body, headers, assertions, and iteration count. `fixturePath` is a frozen SQL snapshot for the repository's local Compose Postgres, and `replayCassettePath` is a JSON array of recorded external responses. Codepool reads this manifest and both referenced files from the trusted control plane at the base SHA, then sends the resolved content—not checkout credentials—to the sandbox worker. A missing manifest means that repository has no sandbox run. The root Codepool repository includes a working example in `.codepool/sandbox.json` and a local `/api/health` route.

The environment variables remain a local fallback for testing the Codepool repository directly: `SANDBOX_FIXTURE_SNAPSHOT` accepts a local fixture path (or raw fixture text), `SANDBOX_SCENARIOS_JSON` contains the scenario array, and `DAYTONA_REPLAY_CASSETTE` points to the local cassette JSON file. Repository configuration takes precedence over these fallback values. Keep the cassette empty when no external calls are expected; replay misses fail closed. Ordinary reviews continue independently of this optional execution setup.
